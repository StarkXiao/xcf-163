...
import { TIDE_PHASES } from '../src/modules/TideSystem.js';
...
function createRealGame() {
  ...
  game.expedition = { stats: { totalExpeditions: 0 } };
  return game;
}

function advanceToTide(game, tideId) {
  let safety = 20;
  while (game.tideSystem.getCurrentPhase().id !== tideId && safety-- > 0) {
    game.tideSystem.advancePhase();
  }
  return game.tideSystem.getCurrentPhase().id === tideId;
}
...
test('5. 稀有度目标累计统计（重复捕获也被计数）——核心修复验证', () => {
  Storage.clear();
  const game = createRealGame();
  const story = game.storySystem;
  story.startChapter('chapter_03');

  const rareCreature = CREATURES.find(c => c.rarity === RARITY.RARE);
  assert(rareCreature !== undefined,
    '找到稀有生物样本',
    '无法找到稀有生物样本');

  game.inventory.addToBackpack(rareCreature);
  const after1 = story.chapterObjectives['chapter_03']['obj_03_3'];
  log(`ℹ️  第1次捕获稀有后: progress=${after1.progress}/2, inv.rarityCatchStats=${JSON.stringify(game.inventory.rarityCatchStats)}`, 'yellow');
  assert(after1.progress === 1,
    `首次捕获稀有进度=1 (实际=${after1.progress})`,
    '首次捕获稀有进度异常');
  assert(after1.completed === false,
    `仅捕获1次时目标未完成 (completed=${after1.completed})`,
    '目标不应在捕获1次时就完成');

  game.inventory.addToBackpack(rareCreature);
  const after2 = story.chapterObjectives['chapter_03']['obj_03_3'];
  log(`ℹ️  第2次重复捕获同一稀有后: progress=${after2.progress}/2, completed=${after2.completed}`, 'yellow');
  assert(after2.progress >= 2,
    `重复捕获同一稀有后进度>=2 (实际=${after2.progress})`,
    '稀有度目标未统计重复捕获——这正是本次要修复的 bug');
  assert(after2.completed === true,
    '重复捕获同一稀有生物后目标完成',
    '重复捕获未让稀有度目标完成');

  assert(game.inventory.getTotalCaughtByRarityName('稀有') >= 2,
    `Inventory.getTotalCaughtByRarityName('稀有') >= 2 (实际=${game.inventory.getTotalCaughtByRarityName('稀有')})`,
    'Inventory 累计捕获统计方法返回错误');
});

test('6. 金币目标、连击目标、稀有度目标协同', () => {
  Storage.clear();
  const game = createRealGame();
  const story = game.storySystem;
...
test('7. 第一章全目标完成 → 自动完成章节 → 第二章自动启动', () => {
  Storage.clear();
  const game = createRealGame();
  const story = game.storySystem;
  story.startChapter('chapter_01');
...
test('8. StorySystem 存档与读档完整保留状态（含 Inventory 稀有度累计）', () => {
  Storage.clear();
  ...
  const rareC = CREATURES.find(c => c.rarity === RARITY.RARE);
  gameA.inventory.addToBackpack(rareC);
  gameA.inventory.addToBackpack(rareC);

  gameA.storySystem.storyFlags.add('asked_oldworld');

  const invSaved = gameA.inventory.toJSON();
  assert(invSaved.rarityCatchStats && invSaved.rarityCatchStats['稀有'] === 2,
    `Inventory.toJSON 包含 rarityCatchStats={'稀有':2} (实际=${JSON.stringify(invSaved.rarityCatchStats)})`,
    'Inventory 未持久化稀有度累计捕获统计');

  const gameB = createRealGame();
  gameB.inventory.loadData(invSaved);
  assert(gameB.inventory.getTotalCaughtByRarityName('稀有') === 2,
    `读档后累计捕获稀有=2 (实际=${gameB.inventory.getTotalCaughtByRarityName('稀有')})`,
    'Inventory 读档后稀有度累计统计丢失');

  const savedJSON = gameA.storySystem.toJSON();
  log(`ℹ️  Story 存档包含 keys: ${Object.keys(savedJSON).join(', ')}`, 'yellow');
  ...
  const gameC = createRealGame();
  gameC.storySystem.loadData(savedJSON);

  assert(gameC.storySystem.currentChapterId === 'chapter_01',
    '读档后 currentChapterId 一致',
    '读档章节ID不一致');
  assert(gameC.storySystem.storyFlags.has('asked_oldworld'),
    '读档后 storyFlags 已恢复',
    '剧情标记读档丢失');

  const loadedObjs = gameC.storySystem.chapterObjectives['chapter_01'];
  ...
  log('✅ StorySystem + Inventory 存档/读档验证通过', 'green');
});

test('9. 多章节连贯推进（模拟全流程）', () => {
  Storage.clear();
  const game = createRealGame();
  const story = game.storySystem;
  ...
  assert(story.completedChapters.has('chapter_01'),
    '第一章已完成',
    `第一章未完成，目标状态: ${JSON.stringify(story.chapterObjectives['chapter_01'])}`);
  log(`✅ 第一章完成，自动推进至: ${story.currentChapterId}`, 'green');
  ...
  const ok = advanceToTide(game, 'storm_tide');
  assert(ok, '成功切换到风暴潮 phase', '无法切换到风暴潮');
  log(`ℹ️  当前潮位: ${game.tideSystem.getCurrentPhase().name}`, 'yellow');
  for (let i = 0; i < 3; i++) {
    game.stats.catchCount++;
    story.onGameEvent('catch_count');
  }
  assert(story.completedChapters.has('chapter_02'),
    '第二章已完成',
    `第二章未完成，目标状态: ${JSON.stringify(story.chapterObjectives['chapter_02'])}`);
  log(`✅ 第二章完成，自动推进至: ${story.currentChapterId}`, 'green');

  game.expedition.stats.totalExpeditions = 1;
  story.onGameEvent('expedition_complete');

  const uncommons = CREATURES.filter(c => c.rarity === RARITY.UNCOMMON);
  uncommons.slice(0, Math.min(2, uncommons.length)).forEach(c => game.inventory.addToBackpack(c));
  const rares = CREATURES.filter(c => c.rarity === RARITY.RARE);
  const rareSample = rares[0];
  game.inventory.addToBackpack(rareSample);
  game.inventory.addToBackpack(rareSample);

  game.stats.catchCount = 30;
  story.onGameEvent('catch_count');
  CREATURES.filter(c => !game.inventory.collection.has(c.id)).slice(0, 20).forEach(c => game.inventory.addToBackpack(c));
  assert(story.completedChapters.has('chapter_03'),
    '第三章已完成',
    `第三章未完成，目标状态: ${JSON.stringify(story.chapterObjectives['chapter_03'])}`);
  log(`✅ 第三章完成，自动推进至: ${story.currentChapterId}`, 'green');

  const epics = CREATURES.filter(c => c.rarity === RARITY.EPIC);
  if (epics.length > 0) game.inventory.addToBackpack(epics[0]);
  game.stats.comboCount = 10;
  story.onGameEvent('combo_reach', 10);
  game.expedition.stats.totalExpeditions = 3;
  story.onGameEvent('expedition_complete');
  assert(story.completedChapters.has('chapter_04'),
    '第四章已完成',
    `第四章未完成，目标状态: ${JSON.stringify(story.chapterObjectives['chapter_04'])}`);
  log(`✅ 第四章完成，自动推进至: ${story.currentChapterId}`, 'green');

  const legendaries = CREATURES.filter(c => c.rarity === RARITY.LEGENDARY);
  if (legendaries.length > 0) game.inventory.addToBackpack(legendaries[0]);
  if (legendaries.length > 1) game.inventory.addToBackpack(legendaries[1]);
  ...
  log(`📈 Inventory.rarityCatchStats: ${JSON.stringify(game.inventory.rarityCatchStats)}`, 'yellow');

  assert(completedCount >= 4,
    `连贯推进至少完成4章 (实际=${completedCount})`,
    `连贯推进失败，仅完成 ${completedCount}/4 章`);
});

console.log('\n════════════════════════════════════════════════════════');
log(`📋 总计运行 ${testCases} 个测试用例，共 ${totalPassed + totalFailed} 个断言`, 'blue');
if (totalFailed === 0) {
  log(`🎉 全部 ${totalPassed} 个断言通过! 主线剧情系统验证成功（章节推进·广播解锁·对话触发·图鉴目标·稀有度累计·存档读档）。`, 'green');
} else {
  log(`⚠️  ${totalPassed} 通过, ${totalFailed} 失败`, 'red');
}
log('════════════════════════════════════════════════════════\n');
